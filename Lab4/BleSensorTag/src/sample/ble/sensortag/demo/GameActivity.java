...
/*	public void onStart(){
	...
	//not used
	public void userClickYes(String stompType){
    	...
    	if (sensor instanceof TiAccelerometerSensor) {
         ...
         /*  //isStomp = motion.detectStomp(values[1]);
           isKick = motion.detectKick(values[1],values[2]);  
            ...
            if(!isStomp.equalsIgnoreCase("none")){
            	//Log.i("SlowStompDetection", dated +"  : y  :" + values[1]);
            	userClickYes(isStomp);
            }else if (isKick) {
            	userClickNo();
			}else{
				isStomp ="none";
